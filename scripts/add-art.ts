  ...
  try {
    ...
    for (const img of gallery.images) {
      if (img.image_url.startsWith("__TEMP_SRC__")) {
        ...
        const imagePath = dest.replace(/\\/g, "/").replace(/^public\//, "");
...
async function addVariantsFlexible(
  ...
  try {
    ...
    if (imageId) {
      ...
      if (!base) {
        Logger.warning("Image id not found; entering interactive selection.");
      }
    }
    if (!base) {
    ...
    for (const file of variantFiles) {
      ...
      const varImagePath = dest.replace(/\\/g, "/").replace(/^public\//, "");
