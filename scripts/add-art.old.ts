...
// IMPORTANT: Use `artcli.ts` instead, this file is now deprecated but kept for reference and potential future use.
...
async function showArt(slug: string): Promise<void> {
    ...
    Logger.log(yaml.dump(found.data, { indent: 2, lineWidth: -1 }));
}
...
async function createArt(args: string[]): Promise<void> {
    ...
    try {
        ...
        if (isCharacter) {
            ...
            const rel = await rl.question(
                "Related characters (comma, optional): ",
            );
        ...
        for (const img of gallery.images) {
            if (img.image_url.startsWith("__TEMP_SRC__")) {
                ...
                const imagePath = dest
                    .replace(/\\/g, "/")
                    .replace(/^public\//, "");
                const thumbPath = thumbDest
                    .replace(/\\/g, "/")
                    .replace(/^public\//, "");
                img.image_url =
                    "/" +
                    imagePath.split("/").map(encodeURIComponent).join("/");
                img.thumbnail_url =
                    "/" +
                    thumbPath.split("/").map(encodeURIComponent).join("/");
            }
            if (img.variants?.length) {
                for (const v of img.variants) {
                    if (!v.image_url.startsWith("__TEMP_SRC__")) continue;
                    const originalVarSrc = v.image_url.replace(
                        "__TEMP_SRC__",
                        "",
                    );
                    ...
                    const thumbVar = join(
                        thumbsDir,
                        `${img.id}__${slugPart}.webp`,
                    );
                    await generateThumbnail(destVar, thumbVar);
                    const varImagePath = destVar
                        .replace(/\\/g, "/")
                        .replace(/^public\//, "");
                    const varThumbPath = thumbVar
                        .replace(/\\/g, "/")
                        .replace(/^public\//, "");
                    v.image_url =
                        "/" +
        ...
        if (related_characters?.length)
            art.related_characters = related_characters;
...
async function addVariantsFlexible(
    ...
    try {
        ...
        if (imageId) {
            ...
            if (!base) {
                Logger.warning(
                    "Image id not found; entering interactive selection.",
                );
            }
        }
        if (!base) {
            ...
            if (
                isNaN(choice) ||
                choice < 0 ||
                choice >= found.data.images.length
            ) {
        ...
        for (const file of variantFiles) {
            ...
            const varImagePath = dest
                .replace(/\\/g, "/")
                .replace(/^public\//, "");
            const varThumbPath = vThumbDest
                .replace(/\\/g, "/")
                .replace(/^public\//, "");
            base!.variants.push({
                image_url:
                    "/" +
                    varImagePath.split("/").map(encodeURIComponent).join("/"),
                thumbnail_url:
                    "/" +
                    varThumbPath.split("/").map(encodeURIComponent).join("/"),
                label: label || undefined,
                alt: alt || undefined,
            });
...
async function removeImage(
    ...
    if (withFiles) {
        ...
        if (removed?.thumbnail_url)
            paths.push("public" + removed.thumbnail_url);
        if (removed?.variants) {
...
async function removeVariant(
    ...
    if (withFiles) {
        ...
        if (removed?.thumbnail_url)
            targets.push("public" + removed.thumbnail_url);
...
async function updateMeta(slug: string): Promise<void> {
    ...
    try {
        ...
        const desc =
            (await rl.question(`Description (leave blank keep) : `)) ||
            d.description;
            ...
            if (img.thumbnail_url) {
                try {
                    ...
                    if (await thumbFile.exists())
                        await rm("public" + img.thumbnail_url);
                } catch {}
            }
            if (img.variants) {
                for (const v of img.variants) {
                    try {
                        ...
                        if (await vFile.exists())
                            await rm("public" + v.image_url);
                    } catch {}
                    if (v.thumbnail_url) {
                        try {
                            const vThumbFile = Bun.file(
                                "public" + v.thumbnail_url,
                            );
