...
async function updateYamlThumbnailUrl(
  ...
  try {
    ...
    if (!yamlPath) {
      if (!miniLog) {
        Logger.warning(`No YAML file found for image: ${Logger.fmtBold(imagePath)}`);
      }
      return false;
    }
    ...
    if (!data || !data.thumbnail_url) {
      if (!miniLog) {
        Logger.warning(`No thumbnail_url field in: ${Logger.fmtBold(yamlPath)}`);
      }
      return false;
    }

    // Generate new thumbnail URL (relative to public/)
    const relativePath = relative("public", newThumbnailPath).replace(/\\/g, "/");
    const newThumbnailUrl = "/" + relativePath.split("/").map(encodeURIComponent).join("/");

    // Generate the expected image URL for matching (both encoded and non-encoded)
    const imageRelativePath = relative("public", imagePath).replace(/\\/g, "/");
    const imageUrlEncoded = "/" + imageRelativePath.split("/").map(encodeURIComponent).join("/");
    ...
    if (data.image_url === imageUrlNonEncoded && data.image_url !== imageUrlEncoded) {
    ...
    if (Array.isArray(data.images)) {
      for (const img of data.images) {
        if (img && typeof img === "object") {
          ...
          if (img.image_url === imageUrlEncoded || img.image_url === imageUrlNonEncoded) {
            ...
            if (img.image_url === imageUrlNonEncoded && img.image_url !== imageUrlEncoded) {
    ...
    if (!miniLog) {
      Logger.success(
        `Updated YAML: ${Logger.fmtBold(yamlPath)} -> ${Logger.fmtBold(newThumbnailUrl)}`,
      );
    }
    return true;
  } catch (error) {
    if (!miniLog) {
      Logger.error(`Failed to update YAML for ${Logger.fmtBold(imagePath)}: ${error}`);
    }
    return false;
  }
}
...
async function createThumbnail(
  ...
  try {
    ...
    if (!miniLog) {
      Logger.success(`Created thumbnail: ${Logger.fmtBold(outputPath)}`);
    }
  } catch (error) {
    if (!miniLog) {
      Logger.error(`Failed to create thumbnail for ${Logger.fmtBold(inputPath)}: ${error}`);
    }
    throw error;
  }
}
...
async function regenerateAllThumbnails(
  ...
  if (dryRun) {
    if (!miniLog) {
      ...
      Logger.info(`Total images that would be processed: ${allImages.length}`);
    } else {
      Logger.info(`Dry run complete: ${allImages.length} images would be processed.`);
    }
    return;
  }
  ...
  for (const imagePath of allImages) {
    try {
      ...
      if (!existsSync(imagePath)) {
        if (!miniLog) {
          Logger.error(`File not found: ${Logger.fmtBold(imagePath)}`);
        }
        errors++;
        continue;
      }
      ...
      if (!stat.isFile()) {
        if (!miniLog) {
          Logger.error(`Not a file: ${Logger.fmtBold(imagePath)}`);
        }
        errors++;
        continue;
      }
      ...
      if (!SUPPORTED_EXTENSIONS.includes(ext)) {
        if (!miniLog) {
          Logger.error(`Unsupported format: ${Logger.fmtBold(imagePath)}`);
        }
        errors++;
        continue;
      }
      ...
      if (fileName.startsWith("thumb_")) {
        if (!miniLog) {
          Logger.dim(`Skipping thumbnail file: ${Logger.fmtBold(imagePath)}`);
        }
        skipped++;
        continue;
      }
      ...
      if (!existsSync(thumbnailDir)) {
        ...
        if (!miniLog) {
          Logger.success(`Created thumbnails directory: ${Logger.fmtBold(thumbnailDir)}`);
        }
      }
      ...
      if (existsSync(thumbnailPath) && !force) {
        ...
        if (thumbnailStat.mtime >= originalStat.mtime) {
          if (!miniLog) {
            Logger.dim(`Thumbnail is up-to-date: ${Logger.fmtBold(thumbnailPath)}`);
          }
          skipped++;
          continue;
        }
      ...
      const yamlUpdateResult = await updateYamlThumbnailUrl(imagePath, thumbnailPath, miniLog);
      if (yamlUpdateResult) {
        yamlUpdated++;
      }
    } catch (error) {
      errors++;
      if (!miniLog) {
        Logger.error(`Error processing ${Logger.fmtBold(imagePath)}: ${error}`);
      }
    }
  }
...
async function main() {
  ...
  if (force && !dryRun && !miniLog) {
    Logger.info("FORCE MODE: Regenerating all thumbnails regardless of modification times");
  }
