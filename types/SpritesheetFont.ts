...
export class SpritesheetFont {
    ...
    private characterPositions: Record<
        string,
    ...
    private calculateCharacterPositions(): void {
        for (const [styleName, styleData] of Object.entries(
            this.fontData.styles,
        )) {
            ...
            for (let i = 0; i < styleData.characters.length; i++) {
                ...
                if (typeof char !== "undefined") {
                    const x =
                        (i % styleData.spritesheetColumns) *
                        styleData.characterWidth;
                    const y =
                        Math.floor(i / styleData.spritesheetColumns) *
                        styleData.characterHeight;
    ...
    setStyle(styleName: string): void {
        if (this.fontData.styles[styleName]) {
            ...
            console.warn(
                `Font style "${styleName}" not found. Available styles: ${Object.keys(this.fontData.styles).join(", ")}`,
            );
        }
    }
    ...
    getCharacterStyle(
        ...
        if (!styleData) {
            console.warn(
                `Font style "${style}" not found. Using default style.`,
            );
            const defaultStyleData =
                this.fontData.styles[this.fontData.defaultStyle];
            return this.getCharacterStyle(
                char,
                scale,
                spacing,
                size,
                nextChar,
                this.fontData.defaultStyle,
            );
        }

        const { x, y } = this.characterPositions[style]?.[char] || {
            x: 0,
            y: 0,
        };
        const bounds = styleData.characterBounds[char] ||
            styleData.characterBounds[" "] || {
                left: 0,
                top: 0,
                width: styleData.characterWidth,
                height: styleData.characterHeight,
                baselineOffset: 0,
            };
        ...
        if (nextChar) {
            kerningAdjustment =
                this.getKerning(char, nextChar, style) * finalScale;
        }
        ...
        return {
            ...
            transform:
                scaledBaselineOffset !== 0
                    ? size
                        ? `translateY(${scaledBaselineOffset}${size.replace(parseFloat(size).toString(), "")})`
                        : `translateY(${scaledBaselineOffset}px)`
                    : undefined,
    ...
    getSpaceStyle(
        ...
        const bounds = styleData.characterBounds[" "] || {
            left: 0,
            top: 0,
            width: 32,
            height: 64,
    ...
    splitTextIntoLines(text: string, styleName?: string): string[][] {
        ...
        return text.split("\n").map((line) => {
            return line
                ...
                .filter(
                    (char) =>
                        styleData.characters.includes(char) || char === " ",
                );
        });
    }
    ...
    getTextStyles(
        ...
        size?: string,
        styleName?: string,
    ): (CharacterStyle | SpaceStyle)[] {
        const style = styleName || this.currentStyle;
        const chars = text.split("");
        return chars.map((char, index) => {
            const nextChar =
                index < chars.length - 1 ? chars[index + 1] : undefined;
