      ...
      <div class="status-text">
        ...
        <template
          v-if="
            ...
            (status?.is_playing && status.item?.type === 'track')
          "
        >
          ...
          <div>
            ...
            <a
              ...
              ><b>{{ status.item?.name }}</b></a> by
              <a class="link" :href="status.item?.artists[0]?.external_urls.spotify ?? undefined" target="_blank" rel="noopener noreferrer"
                ><b>{{ status.item?.artists[0]?.name }}</b></a>
                <template v-if="status.item?.artists.length > 1"> (+{{ status.item?.artists.length - 1 }})</template>
          </div>
        </template>
        <template
          v-if="
          ...
          "
        >
          <span v-if="status.emoji" style="display: flex; align-items: center; justify-content: center; width: 24px; height: 24px; font-size: 16px;">{{ (status as EcholotlStatus).emoji }}</span>
          <Icon v-else icon="info" />
          <div>{{ (status as EcholotlStatus).text }}</div>
          <div>
            ({{ getCreatedAtRelativeTime((status as EcholotlStatus).createdAt) }})
          </div>
        </template>
      </div>
...
const status = ref<CurrentlyPlayingResponse | EcholotlStatus | "loading" | "error" | null>("loading");
...
const isEcholotlStatus = (
  value: CurrentlyPlayingResponse | EcholotlStatus | "loading" | "error" | null,
): value is EcholotlStatus => {
  return value !== null && value !== "loading" && value !== "error" && "text" in value;
};
...
const fetchSpotifyStatus: () => Promise<CurrentlyPlayingResponse | null> = async () => {
  try {
...
const refreshStatus = async () => {
  ...
  if (spotifyResponse && spotifyResponse.is_playing && spotifyResponse.item?.type === "track") {
